...
========================
Output stochasticity test for Claude Opus 4.5 and optionally Gemini 2.5 Pro.

Uses three independent runs on 30 randomly selected validation reports.
Measures output consistency at temperature=0.

Expected output verified on study data:
  Claude Opus 4.5: 29/30 identical (96.7%)

NOT: This script can analyze precomputed test data
     or make live API calls if API keys are available.

Usage (precomputed data):
    python 09_stochasticity_test.py

Usage (live API for both models):
...
def analyze_precomputed(filepath: str, model_name: str):
    """Analyze precomputed stochasticity data."""
...
def run_live_claude(df_val: pd.DataFrame, val_ids_30: list, output_file: str):
    """Live Claude API calls with three independent runs."""
...
def main():
    ...
    parser.add_argument('--live', action='store_true',
                        help='Run live API calls; API keys required')
    args = parser.parse_args()

    print("=" * 65)
    print("OUTPUT STOCHASTICITY TEST [REFINE 3.1, 3.3]")
    print(f"N={N_REPORTS} reports x {N_RUNS} independent runs | temperature=0")
    print("=" * 65)
    ...
    if args.live:
        ...
        print(f"\nRunning LIVE Claude API calls ({N_REPORTS} x {N_RUNS} = "
        ...
        if not os.path.exists(stoch_file):
            print(f"\nERROR: {stoch_file} not found.")
            print("For live API use: python 09_stochasticity_test.py --live")
            return
    ...
    expected_n = 29
    ok = c_n == expected_n
    print(f"  {'OK' if ok else 'ERROR'} Claude: {c_n}/{N_REPORTS} = "
