...
=========================
Phrase-level hallucination kontrolü — Claude Opus 4.5 VE Gemini 2.5 Pro.

Her iki modelin döndürdüğü key_phrases listesinin kaynak rapor metninde
verbatim (birebir) bulunup bulunmadığını doğrular.

Sınırlamalar:
  - Sadece "kelime uydurma" hatasını ölçer.
  - OpenI raporlarındaki XXXX redaksiyon göz önünde bulundurulur:
    "XXXX" içeren raporlarda partial match beklenir, bu fabrikasyon değildir.
  - Genel klinik veya semantik halüsinasyon yokluğunu KANITLAMAZ.

Beklenen çıktılar (gerçek veri ile doğrulandı):
  Claude:  0/184 phrase fabricated (0%) — 70 rapor
  Gemini:  0/134 phrase fabricated (0%), 3 partial (XXXX nedeniyle)

NOT: "XXXX" anonymized terms: OpenI'da kişisel veriler XXXX ile değiştirilmiş.
     Bu nedenle bazı phrase'ler tam eşleşme yerine partial olarak görünebilir.
...
def check_phrase(phrase: str, report_text: str) -> str:
    ...
    # 1. Verbatim (birebir eşleşme)
    if phrase_low in text_low:
        return 'verbatim'

    # 2. Partial match (ilk iki kelime — XXXX redaksiyon için)
    ...
    # 3. XXXX redaksiyon kontrolü (OpenI anonimizasyonu)
...
def print_report(model_name: str, stats: dict, details: list):
    ...
    print(f"  Fabricated (hallucinated):{stats['fabricated']}  ({rate_fab:.1f}%)")

    if stats['fabricated'] == 0:
        print(f"\n  ✓ RESULT: 0/{total} fabricated (0%) — NO HALLUCINATIONS")
    else:
        print(f"\n  ✗ RESULT: {stats['fabricated']}/{total} fabricated")
        for d in details:
