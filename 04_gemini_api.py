...
04_gemini_api.py
================
Gemini 2.5 Pro inference for ordinal linguistic uncertainty grading.

Path-2 revision note:
The primary output is uncertainty_level mapped to 0-3. The 0-10 uncertainty_score
field is retained only as a secondary descriptive output.
...
def analyze_report(client, report_text: str) -> dict:
    for attempt in range(MAX_RETRIES):
        try:
            ...
            result["gemini_score"] = LEVEL_MAP.get(level, 0)
...
def main():
    ...
    parser.add_argument("--output", default="gemini_validation_100.json", help="Output JSON path")
    args = parser.parse_args()

    api_key = os.environ.get("GEMINI_API_KEY") or getpass.getpass("Gemini API key: ")
    ...
    for i, row in df.iterrows():
        ...
        print(f"[{i+1:4d}/{len(df)}] ID={report_id} score={result['gemini_score']} level={result['uncertainty_level']}")
        if (i + 1) % 10 == 0:
