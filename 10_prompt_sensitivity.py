...
50 validation raporu üzerinde 3 farklı prompt varyantının
pairwise kappa uyumunu ölçer.

Prompt varyantları:
  v1 (original):   Tam prompt (Supplementary 2)
  v2 (short):      Kısaltılmış direktif versiyon
...
Score: none=0, low=1, moderate=2, high=3.
...
Step 4: Output your final answer as JSON only.
...
def call_claude(client, prompt: str) -> int:
    """Claude API çağrısı, integer score döndür."""
...
def run_live_prompt_sensitivity(df_val: pd.DataFrame, ids_50: list, output_file: str):
    """3 prompt varyantı ile canlı API çağrısı."""
...
def main():
    ...
    parser.add_argument('--live', action='store_true',
                        help='Canlı API çağrısı yap')
    args = parser.parse_args()

    print("=" * 65)
    print("PROMPT SENSITIVITY ANALYSIS [REFINE 2.1, 5.6]")
    print(f"N={N_REPORTS} reports × 3 prompt variants | temperature=0")
    print("=" * 65)
    print("\nPrompt variants:")
    ...
    if args.live:
        ...
        print(f"\nRunning LIVE API calls ({N_REPORTS} × 3 = {N_REPORTS*3})...")
        df_sens = run_live_prompt_sensitivity(df_val, ids_50, sens_file)
    else:
        if not os.path.exists(sens_file):
            print(f"\nERROR: {sens_file} bulunamadı.")
            print("Canlı API için: python 10_prompt_sensitivity.py --live")
            return
